...
function TodoPage() {
  ...
  const addTodo = () => {
    if (input.trim()) {
      setTodos([...todos, { id: Date.now(), text: input, completed: false, category, date }]);
      setInput('');
    }
  };
      ...
      <nav className="todo-navbar">
        <div className="nav-container">
          ...
          <div className="nav-stats">
            <div className={`nav-stat ${filter === 'pending' ? 'active' : ''}`} onClick={() => setFilter('pending')}>
              <span>⏳</span> Pending: {pendingTodos.length}
            </div>
            <div className={`nav-stat ${filter === 'completed' ? 'active' : ''}`} onClick={() => setFilter('completed')}>
              <span>✅</span> Completed: {completedTodos.length}
            </div>
          </div>
                    ...
                    <ul className="todo-list">
                      {pendingGrouped[cat].map(todo => (
                        <li key={todo.id} className="todo-item">
                          <span className="todo-text">{todo.text} - {new Date(todo.date).toLocaleDateString()}</span>
                          <div className="todo-actions">
                            <button onClick={() => toggleTodo(todo.id)} className="toggle-btn">Complete</button>
                    ...
                    <ul className="todo-list">
                      {completedGrouped[cat].map(todo => (
                        <li key={todo.id} className="todo-item completed">
                          <span className="todo-text">{todo.text} - {new Date(todo.date).toLocaleDateString()}</span>
                          <div className="todo-actions">
                            <button onClick={() => toggleTodo(todo.id)} className="toggle-btn">Undo</button>
                            <button onClick={() => deleteTodo(todo.id)} className="delete-btn">Delete</button>
                          </div>
                        </li>
                      ))}
